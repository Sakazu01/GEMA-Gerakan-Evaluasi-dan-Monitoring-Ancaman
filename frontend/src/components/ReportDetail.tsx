...
export function ReportDetail({ id }: { id: string }) {
  ...
  const report = reports.find((item) => item.id === id && item.status === "active");
  ...
  if (!report) {
    return (
      <main className="mx-auto max-w-3xl px-4 pb-28 pt-8 sm:px-6">
        ...
        <p className="mt-2 text-slate-700">Laporan DEMO yang baru dikirim hanya tersedia selama sesi halaman ini.</p>
    ...
    <main className="mx-auto max-w-3xl px-4 pb-28 pt-8 sm:px-6">
      ...
      <Card className="mt-6">
        <dl className="space-y-4 text-slate-900">
          <div><dt className="font-semibold">Keparahan</dt><dd className="capitalize">{report.severity}</dd></div>
          ...
          <div><dt className="font-semibold">Kabar bantuan</dt><dd>
            {report.help_status === "terlihat" ? "Bantuan dilaporkan terlihat oleh warga" :
              ...
              "Belum ada konfirmasi warga yang cukup"}
          </dd></div>
        </dl>
      </Card>
    </main>
